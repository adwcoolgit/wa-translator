...
""";
    ...
    private static ProviderHealthCheckResult CreateFailureResultFromException(
        ...
        long startedAt)
    {
        var normalizedError = new ProviderErrorNormalizer().NormalizeException(exception);
        return CreateErrorResult(request, provider, normalizedError, startedAt);
    }
    ...
    private static ProviderHealthCheckResult CreateErrorResult(
        ...
        long startedAt)
    {
        var severity = errorCode is "PROVIDER_TIMEOUT" or "PROVIDER_RATE_LIMIT" ? "warning" : "blocking";
        return CreateErrorResult(
            request,
            provider,
            new ProviderErrorNormalizer().CreateError(errorCode, recoveryAction, "provider", severity),
            startedAt);
    }
